  ...
  (() => {
    ...
    function renderStaticTop() {
      ...
      document.querySelector('#done-grid').innerHTML = history.map(s => `<div class="done"><div class="small">${s.yr} · ${s.sem} · ${model.completed([s]).total}cr</div><strong>${s.average.toFixed(1)}</strong><div class="small">成績單學期平均</div></div>`).join('');
    }
    const labels = { gened: '共同必修 △', major: '專業必修 ▲', elec: '專業選修 ★', free: '跨域+自由' };
    function renderHistory() {
      document.querySelector('#history').innerHTML = `<p class="history-intro">四學期完整紀錄 · 課名、階段、學分、成績與學期平均依提供的成績單。W 撤選不計入取得學分；零學分課程仍保留通過紀錄。</p>` + history.map(s => `<details class="card history-card" open><summary><strong>${s.sem} <span class="small">${s.yr}</span></strong><span>${model.completed([s]).total} cr 已取得 · 平均 ${s.average.toFixed(1)}</span></summary><div class="table-wrap"><table><caption class="hide">${s.sem} 修課紀錄</caption><thead><tr><th scope="col">課程</th><th scope="col">修別</th><th scope="col">學分</th><th scope="col">成績</th><th scope="col">取得</th></tr></thead><tbody>${s.courses.map(c => `<tr class="${c.score === 'W' ? 'retake' : ''}"><td>${escapeHtml(c.name)}<span class="course-meta">${c.number} · ${c.code} · 階段 ${c.stage}${c.emi ? ' · EMI' : ''}</span></td><td>${c.type}</td><td>${c.credits}</td><td>${c.score === 'W' ? 'W 撤選' : c.score}</td><td>${model.earned(c)}</td></tr>`).join('')}</tbody></table></div><div class="history-note">來源：${s.source} · 列印 ${s.printed}（民國） · 成績單修習 ${s.reportedCredits}cr / 實得 ${s.reportedEarned}cr · 操行 ${s.conduct}<br>必＝必修、選＝選修、通＝通識；EMI＝全英語授課。${s.sem === '114-2' ? ' 成績單修習 19cr 已排除兩門撤選的 6cr。' : ''}</div></details>`).join('');
    }
